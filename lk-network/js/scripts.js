$(document).ready(function(){
  ...
  $(".soft-pull").click(function(e){
   e.preventDefault();
   $('.softpull-load').show().delay('1200').fadeOut('fast', function(){
     $('.pre-soft-pull').hide().remove();
     $('.after-soft-pull').show();
...
// -- CONTROLS FOR FINOVATE DEMO -- //
  ...
  $('.degree .submit').on('click', function(){
    $('#edge').delay('500').slideUp(function(){
      $('#edge').remove();
    });
  });
  ...
  $('.ach-authorization').on('click', function(){
    if($(this).is(':checked')) {
      ...
      $('li.setup-payment-options').addClass('active').wrapInner('<span class="on"></span>');
    } else {
  ...
  $('#payoff-information .gathering-btn').click(function(){
    ...
    $('.payoff-close').slideUp();
    $('.payoff-success').slideDown();
  });
  ...
  // keeps the sidebar locked on scroll
  $(window).scroll(function(){
    if ($(window).scrollTop() > 100){
        $(".candybar").addClass('candybar-fixed');
    } else {
        $(".candybar").removeClass('candybar-fixed');
    }
});
